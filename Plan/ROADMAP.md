...
- ATHENA, JARVIS and JANUS are independent intelligences.
...
- Root README and AI portal use the corrected identities.
- Plan collaboration model and implementation matrix are corrected.
...
- Create JANUS manifest, identity boundary and local runtime package.
- Build read-only Windows UIA/Win32/accessibility application mapper.
...
- JANUS can map at least four applications without mutation.
...
- Disposable/sandbox project cloning.
- One-action-at-a-time controlled exploration.
- State-transition recorder and rollback verification.
- Capability-hypothesis ledger.
- Skill Capsule schema, registry, version range and deprecation.
- Human-demonstration parser that removes accidental cursor paths.

Exit criteria:

- One unfamiliar application workflow is discovered, reproduced and verified.
...
- Universal object/material/behavior schema.
- Reality and validation classes.
- Scene graph, world snapshots and replay.
- Conventional physics/render engine integration.
- Engineering import/export package.
- Persistent R&D workspace prototype.
- ATHENA, JARVIS and JANUS interface contracts.

Exit criteria:

- A project world can be created, branched, restored and exported.
...
- Extract AI-OS v13 modules and invariants.
- Extract Cognitive Orchestrator attention/memory/agent contracts.
- Extract Athena Memory Architecture into schemas.
- Extract Odysseus ontology migration tasks.
...
- Every Plan artifact has status, provenance, canonical/mirror designation and next implementation target.
...
- Keep Aegis authority cryptographic/logical and independently verifiable.
...
- Register the 13-sheet measured vector package and source provenance.
- Freeze P2 concept dimensions only as a modelling baseline.
- Select candidate optical engine, surface/phosphor and sensor technologies.
- Perform optical ray tracing, thermal modelling, structural FEA and safety review.
- Define telemetry, frame-proof and calibration protocols.
- Build non-Lightcore display/sensor prototype first.

Exit criteria:

- Selected components replace placeholders.
...
- Coordinate-frame and transform schemas.
- TV/tower/headset/controller time synchronization.
- Camera/LiDAR/depth/RF confidence fusion.
- Relational skeleton constraints and human-state reconstruction.
- Occlusion, disagreement and failure-state metrics.
- OpenXR/SteamVR/OSC virtual-tracker bridge.
- Privacy modes: presence, motion, intent/risk, familiarity and explicit identity.

Exit criteria:

- Markerless pose passes named accuracy/latency/occlusion tests.
...
- Choose first deployment use case and reduced sensor set.
- Implement deterministic timing, calibration and raw evidence retention.
...
- One reference room/material set is scanned, registered and uncertainty-labelled.
...
- Each profile has measurable interfaces, risk register and independent benchmark.
...
- Unified status and authority console.
- JARVIS dialogue and personal orchestration interface.
- JANUS action preview, evidence and emergency-stop interface.
- OASIS project-space navigation.
- Plugin mirror/index health and source-of-truth indicators.
- Compact context refresh for new sessions and replacement models.

Exit criteria:

- Human can initiate, inspect, approve, interrupt, replay and audit work across systems.
- Every result shows system identity, evidence state, authority and canonical writeback location.
