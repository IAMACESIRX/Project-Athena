...
module nvisc_livehot_controller_stub #(
    parameter int MEM_WORDS = 256,
    parameter logic [63:0] BASE_ADDR = 64'h0000_0000_0000_3000
)(
    input  logic clk,
    input  logic rst_n,
    ...
    always_comb begin
        in_range_c = (req_paddr_i >= BASE_ADDR) && (word_index_full_c < 64'(MEM_WORDS));
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (req_valid_i) begin
                ...
                if (!op_ok_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0A01;
                end else if (!size_ok_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0A02;
                end else if (!aligned_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0A03;
                end else if (!in_range_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0A04;
