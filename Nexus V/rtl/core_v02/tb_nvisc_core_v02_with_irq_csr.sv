...
module tb_nvisc_core_v02_with_irq_csr #(
    ...
    // CSR access into IRQ CSR bridge.
    ...
    // CSR bridge -> IRQ/timer control.
    logic        csr_global_irq_enable_w;
    logic        csr_timer_enable_w;
    logic [63:0] csr_timer_compare_w;
    logic [63:0] csr_irq_vector_base_w;
    logic        csr_irq_ack_w;

    // IRQ/timer status.
    logic        timer_pending_w;
    logic        external_pending_w;

    // Trap redirect wiring into core.
    logic        irq_trap_valid_w;
    logic [63:0] irq_trap_vector_w;

    // Unused core data/memory outputs.
    ...
    nvisc_irq_csr_bridge u_irq_csr (
        .clk(clk),
        .rst_n(rst_n),
        ...
        .csr_resp_valid_o(csr_resp_valid_o),
        .csr_rdata_o(csr_rdata_o),
        .csr_fault_o(csr_fault_o),
        .csr_fault_code_o(csr_fault_code_o),

        .cycle_count_i(irq_cycle_count_o),
        .irq_pending_i(irq_pending_o),
        .irq_taken_i(irq_taken_o),
        .irq_vector_i(irq_vector_o),
        .irq_code_i(irq_code_o),

        .global_irq_enable_o(csr_global_irq_enable_w),
        .timer_enable_o(csr_timer_enable_w),
        .timer_compare_o(csr_timer_compare_w),
        .irq_vector_base_o(csr_irq_vector_base_w),
        .irq_ack_o(csr_irq_ack_w),
    ...
    nvisc_interrupt_timer_stub u_irq_timer (
        ...
        .global_irq_enable_i(csr_global_irq_enable_w),
        .timer_enable_i(csr_timer_enable_w),
        .timer_compare_i(csr_timer_compare_w),
        .irq_vector_base_i(csr_irq_vector_base_w),

        .external_irq_i(external_irq_i),
        .irq_ack_i(csr_irq_ack_w),

        .cycle_count_o(irq_cycle_count_o),
        .timer_pending_o(timer_pending_w),
        .external_pending_o(external_pending_w),
