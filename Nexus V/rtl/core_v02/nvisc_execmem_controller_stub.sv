...
module nvisc_execmem_controller_stub #(
    parameter int MEM_WORDS = 256,
    parameter logic [63:0] BASE_ADDR = 64'h0000_0000_0000_2000
)(
    input  logic clk,
    input  logic rst_n,
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (req_valid_i) begin
                ...
                if (!op_ok_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0901;
                end else if (!size_ok_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0902;
                end else if (!aligned_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0903;
                end else if (!in_range_c) begin
                    fault_o      <= 1'b1;
                    fault_code_o <= 16'h0904;
